...
var main = function(files, callback) {
  ...
  async.each(files, function(file, next) {
    ...
    fs.readFile(file, 'utf8', function(error, data) {
      if (error) next(error);
      estraverse.traverse(parse(trimHashbang(data), file), {
  ...
  var parse = function (data, file) {
    var result = '';
    try {
      result = esprima.parse(data);
    } catch (e) {
      throw new Error('parse errro in ' + path.basename(file));
    }
    return result;
  };

  var trimHashbang = function (code) {
    if (code.substring(0, 2) !== '#!') return code;
